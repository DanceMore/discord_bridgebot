...
pub async fn unbridge_all(
    ...
    let results: Vec<ChannelPair> = channel_pairs.filter(channel1.eq(current_channel_id).or(channel2.eq(current_channel_id)))
    .load(connection)
            ...
            for pair in results {
                    ...
                    .execute(connection)
                {
                    Ok(_) => {
                        ...
                        ctx.say(format!("{} Successfully unbridged channel `{}` from channel `{}`", emoji, pair.channel1, pair.channel2)).await?;
                    }
                    Err(_) => {
                        let emoji = emojis::get_by_shortcode("x").unwrap();
                        ctx.say(format!("{} Error unbridging the ChannelID `{}` `{}` `{}`", emoji, pair.id, pair.channel1, pair.channel2)).await?;
                    }
                }
            }
//        },
